...
for row in curs.execute(
    "select dataset, size_bytes, events, creation_date from dataset_size"
):
    datasets[row[0]] = {
        "size": row[1],
        "unique_events": row[2],
...
for row in curs.execute("SELECT dataset, size_bytes, site FROM disk_replicas"):
    if (
        row[2].endswith("_MSS")
        or row[2].endswith("_Buffer")
        or row[2].endswith("_Export")
    ):
        continue
    if "#" in row[0]:
        continue
    row_tier = row[0].rsplit("/", 1)[-1]
    if not fnmatch.fnmatch(row_tier, tier_pattern):
        continue
...
for row in curs.execute(
    "SELECT core_hours, events, read_bytes, dataset, crab_task FROM dataset_popularity WHERE month > ? AND month <= ?",
...
for dataset, size_bytes in list(replicas.items()):
...
for i in range(1, 15):
    ...
    print(
        i,
        "%.1fTB in dataset" % (by_crab_tasks[i]["replica_bytes"] / 1e12),
...
print(
    ...
    "%.1fB unique events" % unique_events,
    "%.2fM hours" % hours,
...
print(
    "Total accesses by CRAB: %.1fB unique events, %.1fTB read"
    % (total_unique_events / 1e9, total_bytes / 1e12)
)
print(
    "Current on-disk replicas not accessed: %.1fTB created before this time period, %.1fTB created during this time period"
    % (unaccessed_old_bytes / 1e12, unaccessed_bytes / 1e12)
)
