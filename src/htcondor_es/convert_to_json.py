...
def get_creation_time_from_taskname(ad):
    ...
    try:
        ...
        return int(
            time.mktime(
                datetime.datetime.strptime(_str_date, "%y%m%d_%H%M%S").timetuple()
            )
        )
...
def convert_to_json(ad, cms=True, return_dict=False, reduce_data=False):
    ...
    # Determine type
    analysis = ("CRAB_Id" in ad) or (
        ad.get("AccountingGroup", "").startswith("analysis.")
    )
...
def commonExitCode(ad):
    ...
    return ad.get(
        "Chirp_WMCore_cmsRun_ExitCode",
        ad.get(
            "JobExitCode", ad.get("Chirp_CRAB3_Job_ExitCode", ad.get("ExitCode", 0))
        ),
    )
